...
def run_pipeline(input_path: str, output_dir: str, market_data_path: str | None = None):
    ...
    if raw_df.empty:
        ...
        return
    logger.info("Ingested %d raw rows from %d files", len(raw_df), raw_df["source_file"].nunique())
    ...
    for idx, row in norm_df.iterrows():
        ...
        for key in ("unit_cost_ex_vat", "unit_cost_inc_vat", "case_cost_ex_vat", "case_cost_inc_vat", "case_qty"):
    ...
    for idx, row in norm_df.iterrows():
        try:
            ...
            if not row_dict.get("ean") or not validate_ean(row_dict["ean"]):
                output_rows.append({**row_dict, "decision": "REJECT",
            ...
            for match in matches:
                try:
                    ...
                    logger.exception("[%s] [ROW_%s] [%s] — match processing error",
            ...
            logger.exception("[%s] [ROW_%s] [%s] — pipeline error",
    ...
    if not output_df.empty:
        ...
        write_excel(output_df, xlsx_path, market_data=market_data)
...
def _process_match(match):
    ...
    # Market price = lower of Buy Box or lowest FBA seller (conservative)
    ...
    # RRP sanity check — if Amazon price is wildly different from supplier RRP,
    # the EAN is likely wrong (matched to a different product)
    ...
    # Real Keepa fee data (if available)
    ...
    # Conservative price — two-pass to solve bootstrapping
