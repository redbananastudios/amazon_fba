#!/usr/bin/env node
/**
 * Phase 5 build for kids-toys
 * Transforms shortlist into final_results.csv with 67 columns.
 */
const fs = require('fs');
...
const INPUT = fs.existsSync(path.join(WORKING, 'kids_toys_phase4_ip_risk.csv'))
  ? path.join(WORKING, 'kids_toys_phase4_ip_risk.csv')
  : fs.existsSync(path.join(WORKING, 'kids_toys_phase3_shortlist.csv'))
    ? path.join(WORKING, 'kids_toys_phase3_shortlist.csv')
    : path.join(BASE, 'kids_toys_phase3_shortlist.csv');
const REJECT_CSV = path.join(WORKING, 'kids_toys_phase5_rejected_private_label.csv');
...
// 67 columns with EAN/UPC/GTIN at the end
const OUT_HEADERS = [
...
fs.writeFileSync(path.join(WORKING, 'kids_toys_final_results.csv'), finalCSV.join('\n') + '\n');
fs.writeFileSync(path.join(BASE, 'kids_toys_final_results.csv'), finalCSV.join('\n') + '\n');
...
fs.writeFileSync(path.join(WORKING, 'kids_toys_phase5_suppliers.csv'), supCSV.join('\n') + '\n');
...
`;
fs.writeFileSync(path.join(WORKING, 'kids_toys_phase5_stats.txt'), statsContent);

const handoff = `# Phase 5 Handoff -- kids-toys
...
`;
fs.writeFileSync(path.join(WORKING, 'kids_toys_phase5_handoff.md'), handoff);

console.log('Phase 5 build complete:');
console.log(`  Final results CSV: ${finalRows.length} products, 67 columns`);
