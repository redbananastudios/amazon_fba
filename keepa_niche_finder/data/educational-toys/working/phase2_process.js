#!/usr/bin/env node
/**
 * Phase 2 processing for educational-toys
...
const INPUT = fs.existsSync(path.join(BASE, 'educational_toys_phase1_filtered.csv'))
  ? path.join(BASE, 'educational_toys_phase1_filtered.csv')
  : path.join(WORKING, 'educational_toys_phase1_filtered.csv');
...
// Subcategory filter (educational-toys only)
// Since we exported all of Toys & Games, filter to educational subcategories
const EDU_SUBCATEGORY_KEYWORDS = [
  'learning', 'education', 'educational', 'steam', 'numeracy',
  'learning computer', 'learning area', 'educational game',
  'science', 'coding', 'stem', 'flash card', 'alphabet', 'phonics', 'maths',
  'geography', 'microscope', 'robotics', 'electronics kit'
];
const CATEGORIES_SUB = col['Categories: Sub'];
const SUBCAT_RANKS = col['Sales Rank: Subcategory Sales Ranks'];

const preSubcatCount = lines.length - 1;
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (EDU_SUBCATEGORY_KEYWORDS.some(k => combined.includes(k))) {
    filteredLines.push(lines[i]);
  }
}
console.log(`Subcategory filter: ${preSubcatCount} -> ${filteredLines.length - 1} (removed ${preSubcatCount - filteredLines.length + 1} non-educational)`);
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Low velocity (below 50 for this niche)
  if (!removed && bought < 50) {
...
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase2_enriched.csv'), enrichedCSV.join('\n') + '\n');

// Hazmat CSV (header + rows)
if (hazmatRows.length > 0) {
  const hazmatCSV = [lines[0]];
  hazmatRows.forEach(r => hazmatCSV.push(r.join(',')));
  fs.writeFileSync(path.join(WORKING, 'educational_toys_phase2_hazmat.csv'), hazmatCSV.join('\n') + '\n');
} else {
  fs.writeFileSync(path.join(WORKING, 'educational_toys_phase2_hazmat.csv'), 'No HAZMAT products found\n');
}

// Gated CSV (empty for now - would need SellerAmp)
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase2_gated.csv'),
  OUT_HEADERS.map(escapeCSV).join(',') + '\nNo gating data available without SellerAmp lookup\n');

// Stats
const afterPrefilter = enrichedRows.length;
const statsContent = `Niche: educational-toys
Date: 2026-03-21
...
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase2_stats.txt'), statsContent);
