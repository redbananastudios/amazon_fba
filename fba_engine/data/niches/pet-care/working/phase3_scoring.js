...
const INPUT = fs.existsSync(path.join(BASE, 'pet_care_phase2_enriched.csv'))
  ? path.join(BASE, 'pet_care_phase2_enriched.csv')
  : path.join(WORKING, 'pet_care_phase2_enriched.csv');
...
fs.writeFileSync(path.join(WORKING, 'pet_care_phase3_scored.csv'), scoredCSV.join('\n') + '\n');

const shortlistCSV = [scoredHeaders.map(esc).join(',')];
shortlist.forEach(r => shortlistCSV.push(r.map(esc).join(',')));
fs.writeFileSync(path.join(WORKING, 'pet_care_phase3_shortlist.csv'), shortlistCSV.join('\n') + '\n');
...
fs.writeFileSync(path.join(WORKING, 'pet_care_phase3_shortlist.json'), JSON.stringify(shortlistJSON, null, 2));
...
fs.writeFileSync(path.join(WORKING, 'pet_care_phase3_stats.txt'), statsContent);

const handoff = `# Phase 3 Handoff -- pet-care
...
## Files
- pet_care_phase3_scored.csv
- pet_care_phase3_shortlist.csv
- pet_care_phase3_shortlist.json
- pet_care_phase3_stats.txt

## Next step
Run Phase 4 IP risk: node data/pet-care/working/phase4_ip_risk.js
`;
fs.writeFileSync(path.join(WORKING, 'pet_care_phase3_handoff.md'), handoff);
