#!/usr/bin/env node
/**
 * Phase 5 build for educational-toys
...
const INPUT = fs.existsSync(path.join(WORKING, 'educational_toys_phase4_ip_risk.csv'))
  ? path.join(WORKING, 'educational_toys_phase4_ip_risk.csv')
  : fs.existsSync(path.join(WORKING, 'educational_toys_phase3_shortlist.csv'))
    ? path.join(WORKING, 'educational_toys_phase3_shortlist.csv')
    : path.join(BASE, 'educational_toys_phase3_shortlist.csv');
const REJECT_CSV = path.join(WORKING, 'educational_toys_phase5_rejected_private_label.csv');
...
const OUT_HEADERS = [
  ...
  // Product codes (for supplier price matching)
  'EAN', 'UPC', 'GTIN'
];
...
fs.writeFileSync(path.join(WORKING, 'educational_toys_final_results.csv'), finalCSV.join('\n') + '\n');
fs.writeFileSync(path.join(BASE, 'educational_toys_final_results.csv'), finalCSV.join('\n') + '\n');
...
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase5_suppliers.csv'), supCSV.join('\n') + '\n');
...
`;
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase5_stats.txt'), statsContent);

const handoff = `# Phase 5 Handoff -- educational-toys
...
`;
fs.writeFileSync(path.join(WORKING, 'educational_toys_phase5_handoff.md'), handoff);

console.log('Phase 5 build complete:');
console.log(`  Final results CSV: ${finalRows.length} products, 64 columns`);
