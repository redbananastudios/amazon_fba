...
const INPUT = fs.existsSync(path.join(BASE, 'sports_goods_phase2_enriched.csv'))
  ? path.join(BASE, 'sports_goods_phase2_enriched.csv')
  : path.join(WORKING, 'sports_goods_phase2_enriched.csv');
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase3_scored.csv'), scoredCSV.join('\n') + '\n');

const shortlistCSV = [scoredHeaders.map(esc).join(',')];
shortlist.forEach(r => shortlistCSV.push(r.map(esc).join(',')));
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase3_shortlist.csv'), shortlistCSV.join('\n') + '\n');
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase3_shortlist.json'), JSON.stringify(shortlistJSON, null, 2));
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase3_stats.txt'), statsContent);
