#!/usr/bin/env node
/**
 * Phase 5 build for sports-goods
...
const INPUT = fs.existsSync(path.join(WORKING, 'sports_goods_phase4_ip_risk.csv'))
  ? path.join(WORKING, 'sports_goods_phase4_ip_risk.csv')
  : fs.existsSync(path.join(WORKING, 'sports_goods_phase3_shortlist.csv'))
    ? path.join(WORKING, 'sports_goods_phase3_shortlist.csv')
    : path.join(BASE, 'sports_goods_phase3_shortlist.csv');
const REJECT_CSV = path.join(WORKING, 'sports_goods_phase5_rejected_private_label.csv');
...
// Sort: Commercial Priority -> Verdict -> Monthly Gross Profit desc -> Composite desc
finalRows.sort((a, b) => {
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_final_results.csv'), finalCSV.join('\n') + '\n');
fs.writeFileSync(path.join(BASE, 'sports_goods_final_results.csv'), finalCSV.join('\n') + '\n');
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase5_suppliers.csv'), supCSV.join('\n') + '\n');
...
const statsContent = `Niche: sports-goods
...
`;
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase5_stats.txt'), statsContent);

const handoff = `# Phase 5 Handoff -- sports-goods
...
`;
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase5_handoff.md'), handoff);
