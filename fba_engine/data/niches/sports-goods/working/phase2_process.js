#!/usr/bin/env node
/**
 * Phase 2 processing for sports-goods
...
const NICHE = 'sports-goods';
const BASE = path.resolve(__dirname, '..');
const WORKING = path.join(BASE, 'working');
const INPUT = fs.existsSync(path.join(BASE, 'sports_goods_phase1_filtered.csv'))
  ? path.join(BASE, 'sports_goods_phase1_filtered.csv')
  : path.join(WORKING, 'sports_goods_phase1_filtered.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Weight flag
  let weightFlag = 'OK';
  // Check package dimensions for oversize
  ...
  if (dimStr) {
    ...
    // Keepa stores total cm^3 or individual dimensions - try to extract
    if (dims.length >= 3) {
  ...
  // Sports-goods velocity floor: 50
  if (!removed && bought > 0 && bought < 50) {
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase2_enriched.csv'), enrichedCSV.join('\n') + '\n');

if (hazmatRows.length > 0) {
  const hazmatCSV = [lines[0]];
  hazmatRows.forEach(r => hazmatCSV.push(r.join(',')));
  fs.writeFileSync(path.join(WORKING, 'sports_goods_phase2_hazmat.csv'), hazmatCSV.join('\n') + '\n');
} else {
  fs.writeFileSync(path.join(WORKING, 'sports_goods_phase2_hazmat.csv'), 'No HAZMAT products found\n');
}

fs.writeFileSync(path.join(WORKING, 'sports_goods_phase2_gated.csv'),
...
fs.writeFileSync(path.join(WORKING, 'sports_goods_phase2_stats.txt'), statsContent);
